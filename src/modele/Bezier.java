...
public class Bezier extends ObservableModele {
	...
	public void go() {
		for (float i = 0; i <= 1 && !collision; i += pas) {
			...
			oiseau.move(createCurve(ctrl, i).x, createCurve(ctrl, i).y,createCurve(ctrl, (float) (i+(10*pas))));
			setChanged();
			notifyObservers();
			try {
				Thread.sleep(5);
	 ...
	 * @param list_pdc necessaire pour la courbe
	 * @param t = le pas
	...
	/**
	 * Fonction pour determiner si la courbe rencontre une collision
	 * @return collision ou non
	 */
	public boolean has_colision()
	{
		return collision;
	}
}